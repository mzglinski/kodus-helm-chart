...
When `publicUrls.*` is unset, URLs are derived from ingress hostnames. Scheme follows `ingress.*.tls` (`https://` when true, `http://` when false). Explicit `publicUrls.*` overrides are used verbatim.
...
When `global.mcpServerEnabled` is `true`, the chart deploys `kodus-mcp-manager` (Service + Deployment) and wires `API_KODUS_SERVICE_MCP_MANAGER` / `WEB_HOSTNAME_MCP_MANAGER` to the in-cluster hostname. Set `mcpManager.enabled: false` to keep MCP enabled in the API while managing the manager externally.
...
Configure a [GitHub App](https://docs.kodus.io/how_to_deploy/en/platforms/github/github_app) for repository access, webhooks, and installation flow.

| Helm value | Env var(s) | Secret key (`secrets.keys.*`) |
|------------|------------|-------------------------------|
| `config.github.appId` | `API_GITHUB_APP_ID` | — |
| `config.github.clientId` | `GLOBAL_GITHUB_CLIENT_ID` | — |
| `config.github.webOAuthClientId` | `WEB_OAUTH_GITHUB_CLIENT_ID` | — (falls back to `clientId` when empty) |
| `config.github.installUrl` | `WEB_GITHUB_INSTALL_URL` | — |
| — | `API_GITHUB_CLIENT_SECRET` | `githubAppClientSecret` |
| — | `WEB_OAUTH_GITHUB_CLIENT_SECRET` | `webOAuthGithubClientSecret` |
| — | `API_GITHUB_PRIVATE_KEY` | `githubAppPrivateKey` (PEM, not base64) |

Use a [GitHub App](https://docs.kodus.io/how_to_deploy/en/platforms/github/github_app) for repository access and webhooks. **Sign in with GitHub** (NextAuth) can use the same app’s Client ID or a separate [GitHub OAuth App](https://docs.github.com/en/apps/oauth-apps/building-oauth-apps/creating-an-oauth-app) via `webOAuthClientId` / `webOAuthGithubClientSecret`.

GitHub App credentials (`API_GITHUB_APP_ID`, `API_GITHUB_PRIVATE_KEY`, and related vars) are wired into **api** and **worker** Deployments. The worker processes queued webhook jobs and calls `GithubService` — omitting these vars on worker causes `GitHub App is not configured` errors at runtime. The **web** Deployment only receives OAuth/install URL vars for NextAuth; **webhooks** receives webhook URL config via `backendConfigEnv` but does not call the GitHub App API.

The API webhook URL is derived from ingress (`API_GITHUB_CODE_MANAGEMENT_WEBHOOK`). Store the client secret and PEM private key in the application Secret; the private key must be the decoded PEM (Kubernetes stores it as a secret value — do not double-base64-encode).
...
config:
  github:
    appId: "123456"
    clientId: "Iv1.abcdef..."
    webOAuthClientId: "Ov23li..."  # optional OAuth App for NextAuth; omit to reuse clientId
    installUrl: "https://github.com/apps/your-app/installations/new"
secrets:
  existingSecret: kodus-app-secrets
  # github-app-client-secret, web-oauth-github-client-secret, github-app-private-key
```
...
| config.web.tokenDocs.forgejo | string | `""` |  |
